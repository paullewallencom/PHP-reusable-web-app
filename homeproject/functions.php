...
function pf_protect_nonadmin_page() {
    if(basename($_SERVER['SCRIPT_NAME']) != "index.php") {
        echo "<h1>Error</h1>";
        echo "You cannot access this page directly. Please go to the
main project pages directly.";
        exit;
    }
}
function pf_protect_admin_page() {
    ...
    if(basename($_SERVER['SCRIPT_NAME'])
            != $config_projectadminfilename) {
        echo "<h1>Error</h1>";
        echo "You cannot access this page directly. Please
go to the admin pages directly.";
        exit;
    }
}
